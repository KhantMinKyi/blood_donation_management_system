        ...
        <div class="row justify-content-center">
            <div class="col-lg-6 col-md-8 col-sm-12">
                ...
                @foreach ($blood_request as $bloodRequest)
                    ...
                    <div class="card">
                        <div class="card-body">
                            @if (isset($bloodRequest->patient))
                                {{ $bloodRequest->patient->name }}
                            @else
                                One User
                            @endif
                            needs your blood donation.

                            <br> <br>
                            <a href='{{ url("/donor/bloodRequestDetail/$bloodRequest->id") }}'
                                class="btn btn-secondary mr-3">Detail</a>
                            <a href='{{ url("/donor/responseRequest/$bloodRequest->id/4") }}'
                                class="btn btn-primary mr-3">Confirm</a>
                            <a href='{{ url("/donor/responseRequest/$bloodRequest->id/3") }}'
                                class="btn btn-light mr-3">Cancel</a>
                        </div>
                    </div>
                @endforeach
                @if ($data == 0)
